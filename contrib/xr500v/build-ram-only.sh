...
build_dir=${BUILD_DIR:-"$source_dir/build/xr500v-ram"}
...
make -C "$source_dir" O="$build_dir" en7512_xr500v_ram_defconfig
make -C "$source_dir" O="$build_dir" -j"$jobs"

config="$build_dir/.config"
elf="$build_dir/u-boot"
raw="$build_dir/u-boot-dtb.bin"
artifact="$artifact_dir/xr500v-u-boot-ram-only.xmodem.bin"
...
for symbol in NET MTD SPI SPL TPL; do
	grep -qx "# CONFIG_${symbol} is not set" "$config"
done
grep -qx '# CONFIG_CMD_SAVEENV is not set' "$config"
grep -qx 'CONFIG_SYS_MEM_TOP_HIDE=0x100000' "$config"
grep -qx 'CONFIG_ENV_IS_NOWHERE=y' "$config"
